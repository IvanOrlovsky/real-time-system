...
export function Cold({ poz, index }: RefridgeratorElementsProps) {
	const { info } = useRefridgeratorContext();

	const currentInfo = info.cold[index];
		...
		<Element {...poz} onClick={showCharts}>
			{chartsData && (
				<ChartModal
					...
					title={`Показатели для элемента Холодный коридор на позиции ${index}`}
				/>
			)}
			<FlexItem>
				T:{" "}
